...
## v3.0 业务模式优化：平台系统账号监控，会员只设关键词 (2026-03-20)

### 核心逻辑调整
- [ ] 引擎配置 API：userConfigs 包含所有有关键词/推送设置的用户（不只是有 tgAccounts 的用户）
- [ ] 引擎配置 API：公共群组模式下，每个用户的全局关键词对所有公共群组生效
- [ ] main.py：PUBLIC_MATCH 模式使用每个会员的全局关键词（无需绑定特定群组）

### 前端导航简化（会员视角）
- [ ] 侧边栏：普通会员隐藏"TG账号"和"群组监控"菜单
- [ ] 侧边栏：管理员显示"公共群组管理"菜单入口
- [ ] 会员仪表盘：突出"关键词设置"和"推送目标群"两个核心操作

### 管理员后台增强
- [ ] 公共群组管理页面：添加群组时支持输入群组链接（自动解析真实 ID）
- [ ] 系统账号管理：标记 TG 账号为"系统账号"（与会员账号区分）
- [ ] 管理员后台：显示各系统账号加入的公共群组数量和状态
...
## v3.6 修复命中记录写入 & 管理后台用户详情 (2026-03-23)

- [x] 修复 hit API 返回空响应及不返回 id 的问题（命中记录未写入数据库）
- [x] 管理后台用户管理页面增加详细信息：会员到期日期、关键词列表、命中数统计（改为表格布局，直接展示关键词数、今日命中、总命中）

## v3.7 部署+分页+命中验证 (2026-03-23)
- [x] 管理后台用户列表增加分页功能（每页 20 条）
- [x] 管理后台用户列表增加搜索功能（按用户名/邮箱）
- [x] 后端 admin.users 接口支持 page/pageSize 参数
- [x] 构建并部署到服务器 luxurvs.com
- [x] 验证服务器端 hit_records 表是否正常写入（共26条，今日16条）
