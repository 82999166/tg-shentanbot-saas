...
- [ ] 连接服务器（72.167.134.119），检查环境，安装 Node.js/Python/PostgreSQL
- [ ] 修复关键词规则页面 - 支持编辑关键词内容/匹配模式/状态
- [ ] 修复群组监控页面 - 支持编辑群组名称/状态
- [ ] 修复消息模板页面 - 支持编辑模板内容
- [ ] 修复 TG 账号管理页面 - 支持编辑账号备注
- [ ] 将代码部署到服务器，配置 .env 环境变量
- [ ] 配置 PostgreSQL 数据库，运行 db:push
- [ ] 构建前端并启动 Web 服务
- [ ] 部署 Python 监控引擎（Pyrogram）
- [ ] 测试完整监控链路（关键词命中 → 通知推送）
