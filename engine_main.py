...
class TDLibAccountWorker:
    ...
    def _subscribe_chats(self):
        """订阅所有对话的消息推送
        先调用 loadChats 确保 TDLib 加载最新对话列表，
        再用 getChats(limit=1000) 一次性获取所有已加入群组 ID，
        注册到 _dialog_cache 触发 TDLib 消息推送。
        """
        logger.info(f"[ACC{self.account_id}] 开始订阅对话列表...")
        # 第一步：调用 loadChats 触发 TDLib 加载对话（只调一次，不循环）
        try:
        ...
        time.sleep(2)
        # 第二步：用 getChats 分页获取所有已加入群组（正确分页）
        total_ids = 0
        offset_order = "9223372036854775807"
        offset_chat_id = 0
        page = 0
        while page < 30:  # 最多30页，每页100个，最多3000个群组
            try:
