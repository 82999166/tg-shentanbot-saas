...
function GroupKeywordsPanel({ groupId, groupTitle }: { groupId: number; groupTitle: string }) {
  ...
  return (
    <div className="mt-3 p-3 bg-slate-50/60 rounded-lg border border-slate-200 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <Select value={matchType} onValueChange={(v) => setMatchType(v as any)}>
          ...
          <SelectContent className="bg-slate-100 border-slate-200">
            <SelectItem value="contains" className="text-slate-600 text-xs">包含</SelectItem>
            <SelectItem value="exact" className="text-slate-600 text-xs">精确</SelectItem>
            <SelectItem value="regex" className="text-slate-600 text-xs">正则</SelectItem>
          </SelectContent>
        </Select>
...
function GroupJoinStatusPanel({ groupId, groupTitle }: { groupId: number; groupTitle: string }) {
  ...
  return (
    <div className="mt-2 p-3 bg-slate-50/60 rounded-lg border border-slate-200 space-y-2">
      <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
        ...
        <div className="space-y-1">
          {statusList.map((s) => (
            <div key={s.accountId} className="flex items-center gap-2 text-xs">
              <span className="text-slate-500 truncate flex-1">{s.tgFirstName || s.tgUsername || s.phone || `账号#${s.accountId}`}</span>
              ...
              {s.joinStatus === "pending" && <Badge className="bg-gray-500/20 text-slate-500 border-gray-500/30 text-xs px-1.5 py-0">待加入</Badge>}
            </div>
          ))}
        </div>
      )}
    ...
    <div className="space-y-4">
      <Card className="bg-slate-50/80 border-slate-200">
        <CardHeader className="pb-3">
          <CardTitle className="text-slate-700 text-sm font-medium flex items-center gap-2">
        ...
        <CardContent className="space-y-4">
          ...
          {adding ? (
            <div className="space-y-3 p-4 bg-white/50 rounded-lg border border-slate-300">
              <h4 className="text-slate-600 text-sm font-medium">添加新公共群组</h4>
              <div>
                <Label className="text-slate-500 text-xs mb-1 block">群组 ID 或 @用户名 *</Label>
                <Input
              ...
              <div>
                <Label className="text-slate-500 text-xs mb-1 block">群组名称（可选）</Label>
                <Input
              ...
              <div>
                <Label className="text-slate-500 text-xs mb-1 block">备注（可选）</Label>
                <Input
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => setAdding(false)} className="border-slate-300 text-slate-600">
                  取消
                </Button>
              </div>
            </div>
            ...
            {!groups || groups.length === 0 ? (
              ...
              groups.map((g) => (
                <div key={g.id} className={`rounded-lg border ${
                  g.isActive ? "bg-white/50 border-slate-300" : "bg-white/20 border-slate-200 opacity-60"
                }`}>
                  <div className="flex items-center justify-between p-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-slate-700 text-sm font-medium truncate">{g.groupTitle || g.groupId}</span>
                        {g.isActive
                    ...
                    <div className="flex items-center gap-1.5 ml-3 shrink-0">
                      <Button
                        ...
                        className="h-7 px-2 text-slate-500 hover:text-blue-400 text-xs"
                        onClick={() => {
                      ...
                      <Button
                        size="sm" variant="ghost"
                        className="h-7 w-7 p-0 text-slate-500 hover:text-yellow-400"
                      ...
                      <div className="flex gap-2 mb-2">
                        <button
                          ...
                          className={`text-xs px-2.5 py-1 rounded-md transition-colors ${
                            expandedTab === "keywords" ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-700"
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`text-xs px-2.5 py-1 rounded-md transition-colors ${
                            expandedTab === "status" ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-700"
                          }`}
                        >
  ...
  return (
    <div className="space-y-6">
      {settingFields.map((group) => (
        <Card key={group.group} className="bg-slate-50/80 border-slate-200">
          <CardHeader className="pb-3">
            <CardTitle className="text-slate-700 text-sm font-medium">{group.group}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {group.fields.map((field) => (
              <div key={field.key}>
                <Label className="text-slate-500 text-xs mb-1 block">{field.label}</Label>
                <Input
                  type={field.type}
      ...
      {settings && settings.length > 0 && (
        <Card className="bg-slate-50/80 border-slate-200">
          <CardHeader className="pb-3">
            <CardTitle className="text-slate-700 text-sm font-medium">当前配置预览</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-1">
              {settings.map((s) => (
                <div key={s.key} className="flex items-center justify-between py-1 border-b border-slate-200/50 last:border-0">
                  <span className="text-slate-500 text-xs font-mono">{s.key}</span>
                  <span className="text-slate-700 text-xs font-mono max-w-[200px] truncate">
    ...
    <div className="space-y-6">
          ...
          {status?.configured ? (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-green-500/10 border border-green-500/30">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">API ID: {status.apiId} · 监控引擎运行中</p>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">请填入 TG API 凭证以启动监控引擎</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">API ID</Label>
            <Input
              type="number"
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">API Hash</Label>
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        <CardHeader className="pb-3">
          <CardTitle className="text-slate-700 text-sm font-medium">使用说明</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-xs text-slate-500">
          ...
          <p>3. 如果引擎状态不更新，请通过 SSH 登录服务器执行 <code className="bg-slate-200 px-1 rounded">pm2 logs ecosystem.engine</code> 查看日志。</p>
        </CardContent>
      </Card>
    </div>
  );
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label className="text-slate-500 text-xs mb-1 block">有效月数</Label>
              <Input
                type="number"
                min={1}
                max={12}
            ...
            <div className="flex gap-1">
              {(["all", "unused", "used", "expired"] as const).map((f) => (
                <Button
                  ...
                  className={`h-6 text-xs px-2 ${codeFilter === f ? "bg-blue-600" : "text-slate-500"}`}
        ...
        <CardContent>
          {!codes || codes.length === 0 ? (
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {codes.map((code) => (
                <div key={code.id} className="flex items-center justify-between py-1.5 border-b border-slate-200/50 last:border-0">
                  <div className="flex items-center gap-2">
                    <Badge
                      className={
                        code.status === "unused"
                          ...
                          : code.status === "used"
                          ? "bg-gray-500/20 text-slate-500 text-xs"
                          : "bg-red-500/20 text-red-300 text-xs"
                      }
                    >
                      {code.status === "unused" ? "未使用" : code.status === "used" ? "已使用" : "已过期"}
                    </Badge>
                    <code className="text-slate-600 font-mono text-xs">{code.code}</code>
                  </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        {(["all", "pending", "completed", "expired"] as const).map((f) => (
          <Button
            ...
            className={`h-7 text-xs ${orderFilter === f ? "bg-blue-600" : "border-slate-300 text-slate-500"}`}
      ...
      {!orders || orders.length === 0 ? (
        ...
        <div className="space-y-2">
          {orders.map((order) => (
            <Card key={order.id} className="bg-slate-50/80 border-slate-200">
              <CardContent className="p-4">
                ...
                {order.redeemCode && (
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-slate-500 text-xs">卡密：</span>
...
function BotConfigTab() {
  ...
  return (
    <div className="space-y-6">
          ...
          {config?.botToken ? (
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">请填入 Bot Token 以启用推送通知</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        <CardHeader className="pb-3">
          <CardTitle className="text-slate-700 text-sm font-medium">Telegram Bot 配置</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300">
            通过{" "}
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">Bot Token</Label>
            <div className="relative">
              <Input
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">推送频道/群组 ID（可选）</Label>
            <Input
              type="text"
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label className="text-slate-500 text-xs mb-1.5 block">告警阈值（健康度分）</Label>
              <Input
                type="number"
                min={0}
                max={100}
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-3">
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">TRC-20 收款地址</Label>
            <Input
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-3">
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">二维码图片 URL</Label>
            <Input
          ...
          {(currentQrUrl || qrPreview) && (
            <div className="mt-2">
              <Label className="text-slate-500 text-xs mb-1.5 block">预览</Label>
              <img
                src={qrPreview || currentQrUrl}
                alt="二维码预览"
                className="w-32 h-32 object-contain rounded border border-slate-300 bg-white"
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-3">
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">价格说明（每行一条）</Label>
            <textarea
              rows={5}
              placeholder={"1月 = 30U\n3月 = 80U\n6月 = 140U\n1年 = 280U"}
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-3">
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">购买客服 TG 用户名（不含 @）</Label>
            <Input
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">支付说明备注</Label>
            <Input
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent>
          <div className="bg-white rounded-lg p-4 text-sm text-slate-700 font-mono whitespace-pre-wrap border border-slate-200">
            {[
              "✅ 价格说明（免费3小时试用）：",
    ...
    <div className="space-y-6">
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">平台名称</Label>
            <Input
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">客服 TG 用户名（不含 @）</Label>
            <Input
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">官方频道链接</Label>
            <Input
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${getValue("anti_spam_enabled") === "false" ? "bg-slate-300" : "bg-blue-600"}`}
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label className="text-slate-500 text-xs mb-1.5 block">每日命中上限（次/人）</Label>
              <Input
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label className="text-slate-500 text-xs mb-1.5 block">接收通知的 TG 账号</Label>
            <Input
...
function SmtpSettingsTab() {
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-slate-50/80 border-slate-200">
        <CardHeader className="pb-3">
          <CardTitle className="text-slate-700 text-sm font-medium">SMTP 邮件服务配置</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300">
          ...
          <div className="flex items-center gap-2 pt-1">
            <input
              ...
              className="w-4 h-4 rounded border-slate-300 bg-slate-100 accent-blue-500"
            />
            <label htmlFor="smtp_secure" className="text-sm text-slate-500 cursor-pointer">
      ...
      <Card className="bg-slate-50/80 border-slate-200">
        <CardHeader className="pb-3">
          <CardTitle className="text-slate-700 text-sm font-medium">常用邮件服务商配置参考</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-xs text-slate-500">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={s.name} className="p-2 rounded bg-white border border-slate-200">
    ...
    <div className="p-6 w-full space-y-6">
      ...
      <Tabs defaultValue="tgapi" className="space-y-4">
        <TabsList className="bg-slate-100 border border-slate-200 flex-wrap h-auto gap-1">
          <TabsTrigger value="tgapi" className="data-[state=active]:bg-blue-600 text-slate-600 data-[state=active]:text-white">
            <Bot className="w-4 h-4 mr-1.5" />
            TG API 凭证
          </TabsTrigger>
          <TabsTrigger value="payment" className="data-[state=active]:bg-blue-600 text-slate-600 data-[state=active]:text-white">
            <DollarSign className="w-4 h-4 mr-1.5" />
            支付配置
          </TabsTrigger>
          <TabsTrigger value="smtp" className="data-[state=active]:bg-blue-600 text-slate-600 data-[state=active]:text-white">
          ...
          <TabsTrigger value="buy" className="text-xs data-[state=active]:bg-blue-600">
            ...
            <TabsTrigger value="sysconfig" className="data-[state=active]:bg-blue-600 text-slate-600 data-[state=active]:text-white">
            <Settings className="w-4 h-4 mr-1.5" />
            系统配置
          </TabsTrigger>
          <TabsTrigger value="tdlib" className="data-[state=active]:bg-blue-600 text-slate-600 data-[state=active]:text-white">
