...
import { Shield, ShieldCheck, ShieldAlert, Clock, Zap, Globe, AlertTriangle } from "lucide-react";
            ...
            <CardContent className="space-y-5">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <Label className="text-xs text-muted-foreground">每日私信上限</Label>
                  <span className={`text-sm font-bold ${riskLevel === "low" ? "text-emerald-400" : riskLevel === "medium" ? "text-amber-400" : "text-red-400"}`}>
                    {form.dailyDmLimit} 条/天
                  </span>
                </div>
                <Slider
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label className="text-xs font-medium">模板轮换</Label>
                  <p className="text-xs text-muted-foreground">随机选用不同模板，避免内容重复</p>
                </div>
                <Switch checked={form.templateRotation} onCheckedChange={(v) => setForm({ ...form, templateRotation: v })} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, minIntervalSeconds: parseInt(e.target.value) || 30 })}
