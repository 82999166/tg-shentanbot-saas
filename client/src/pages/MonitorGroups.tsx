...
import { Monitor, Plus, Trash2, Play, Pause, Users, Hash } from "lucide-react";
...
export default function MonitorGroups() {
  ...
  const createMut = trpc.monitorGroups.create.useMutation({
    onSuccess: () => { utils.monitorGroups.list.invalidate(); toast.success("监控群组添加成功"); setOpen(false); resetForm(); },
  ...
  const [form, setForm] = useState({
    tgGroupId: "",
    groupName: "",
    tgAccountId: undefined as number | undefined,
  ...
  const resetForm = () => setForm({ tgGroupId: "", groupName: "", tgAccountId: undefined, keywordIds: [], notifyTarget: "", autoReply: false, templateId: undefined });
              ...
              <Card key={g.id} className="bg-card border-border hover:border-primary/30 transition-colors">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge className={`text-xs border-0 ${statusColors[g.monitorStatus]}`}>
                      {statusLabels[g.monitorStatus]}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="flex gap-2 pt-1">
                    <Button
                      ...
                      onClick={() => toggleMut.mutate({ id: g.id, status: g.monitorStatus === "active" ? "paused" : "active" })}
                    >
                      {g.monitorStatus === "active" ? (
