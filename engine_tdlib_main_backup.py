...
class TDLibAccountWorker:
        ...
        try:
            ...
            if result.update:
                ...
                if chat_type in ('chatTypeSupergroup', 'chatTypeBasicGroup'):
                    ...
                    # 对 supergroup 额外获取 username 和 member_count
                    if chat_type == 'chatTypeSupergroup' and not username:
                        sg_id = chat_type_obj.get('supergroup_id', 0)
                        if sg_id:
                            try:
                                sg_result = self.tg.call_method('getSupergroup', {'supergroup_id': sg_id})
                                sg_result.wait(timeout=5)
                                if sg_result.update:
                                    sg = sg_result.update
                                    # TDLib 1.8+ usernames 在 supergroup 对象中
                                    sg_usernames = sg.get('usernames', {})
                                    if sg_usernames and isinstance(sg_usernames, dict):
                                        username = sg_usernames.get('editable_username', '') or ''
                                        if not username:
                                            active = sg_usernames.get('active_usernames', [])
                                            if active:
                                                username = active[0]
                                    if not username:
                                        username = sg.get('username', '') or ''
                                    if sg.get('member_count', 0):
                                        member_count = sg.get('member_count', 0)
                            except Exception:
                                pass
                    return {
                        'chatId': str(chat_id),
    ...
    def _handle_scrape_members(self, body: dict) -> dict:
        """采集群组成员 - 双模式：
        1. getSupergroupMembers 获取管理员/机器人
        2. getChatHistory 扫描消息历史提取真实发言用户（超级群组主要方式）
        支持 chatId（数字）或 group（用户名）参数
        """
        chat_id = body.get("chatId")
        group = body.get("group")
        limit = body.get("limit", 200)
        scan_messages = body.get("scanMessages", 2000)  # 扫描消息数量上限

        # ── 解析 chatId ──────────────────────────────────────────
        if not chat_id and group:
            try:
                username = group.lstrip("@").strip()
                if not username:
                    return {"success": False, "error": "group 参数为空"}
                # 方法1：searchPublicChat（重试3次，每次超时15秒）
                for _attempt in range(3):
                    try:
                        result = self.tg.call_method('searchPublicChat', {'username': username})
                        result.wait(timeout=15)
                        if result.update and result.update.get('id'):
                            chat_id = result.update['id']
                            logger.info(f"[ACC{self.account_id}] searchPublicChat 成功: {username} -> {chat_id}")
                            break
                        elif result.error:
                            logger.warning(f"[ACC{self.account_id}] searchPublicChat 错误({_attempt+1}/3): {result.error}")
                    except Exception as e:
                        logger.warning(f"[ACC{self.account_id}] searchPublicChat 异常({_attempt+1}/3): {e}")
                    if not chat_id and _attempt < 2:
                        time.sleep(2)
                # 方法2：从 dialog_cache 中按 username 精确匹配
                if not chat_id:
                    ...
                    with self._dialog_cache_lock:
                        for cid, info in self._dialog_cache.items():
                            if not info:
                                continue
                            cached_username = (info.get('username') or '').lower()
                            cached_title = (info.get('title') or '').lower()
                            if cached_username == username_lower or username_lower in cached_title:
                                chat_id = cid
                                logger.info(f"[ACC{self.account_id}] dialog_cache 匹配: {info.get('title')} -> {cid}")
                                break
                # 方法2.5：searchChatsOnServer 服务器端搜索（比遍历快得多）
                if not chat_id:
                    try:
                        sc_result = self.tg.call_method('searchChatsOnServer', {'query': username, 'limit': 10})
                        sc_result.wait(timeout=15)
                        if sc_result.update:
                            chat_ids_list = sc_result.update.get('chat_ids', [])
                            for cid in chat_ids_list:
                                try:
                                    ci = self._fetch_chat_info(cid)
                                    if ci and (ci.get('username') or '').lower() == username.lower():
                                        chat_id = cid
                                        logger.info(f"[ACC{self.account_id}] searchChatsOnServer 匹配: {ci.get('title')} -> {cid}")
                                        break
                                except Exception:
                                    pass
                    except Exception as e:
                        logger.warning(f"[ACC{self.account_id}] searchChatsOnServer 异常: {e}")
                # 方法3：主动加载 dialog_cache 中 None 条目，再匹配（最多50个，避免超时）
                if not chat_id:
                    username_lower = username.lower()
                    none_ids = []
                    with self._dialog_cache_lock:
                        none_ids = [cid for cid, info in self._dialog_cache.items() if info is None]
                    logger.info(f"[ACC{self.account_id}] 主动加载 {len(none_ids)} 个未缓存对话查找 {username}")
                    for cid in none_ids[:50]:  # 最多加载50个，避免超时
                        try:
                            info = self._fetch_chat_info(cid)
                            if info:
                                with self._dialog_cache_lock:
                                    self._dialog_cache[cid] = info
                                cached_username = (info.get('username') or '').lower()
                                cached_title = (info.get('title') or '').lower()
                                if cached_username == username_lower or username_lower in cached_title:
                                    chat_id = cid
                                    logger.info(f"[ACC{self.account_id}] 主动加载匹配: {info.get('title')} -> {cid}")
                                    break
                            time.sleep(0.1)  # 减少延迟，加快遍历
                        except Exception:
                            pass
                if not chat_id:
                    return {"success": False, "error": f"无法找到群组: {username}"}
            except Exception as e:
                return {"success": False, "error": f"解析群组失败: {str(e)}"}

        if not chat_id:
            return {"success": False, "error": "缺少 chatId 或 group 参数"}

        try:
            chat_id = int(chat_id)
            # openChat 触发消息加载，并预热等待 TDLib 后台下载历史消息
            try:
                self.tg.call_method('openChat', {'chat_id': chat_id}).wait(timeout=5)
            except Exception:
                pass
            # 预热：先调用一次 getChatHistory，触发 TDLib 后台下载
            try:
                preheat = self.tg.call_method('getChatHistory', {
                    'chat_id': chat_id, 'from_message_id': 0, 'offset': 0,
                    'limit': 10, 'only_local': False,
                })
                preheat.wait(timeout=15)
                preheat_count = len((preheat.update or {}).get('messages', []))
                if preheat_count < 5:
                    logger.info(f"[ACC{self.account_id}] 预热获取{preheat_count}条消息，等待TDLib后台下载...")
                    time.sleep(8)  # 等待 TDLib 后台下载更多历史
            except Exception:
                time.sleep(5)

            # 用 dict 去重，key=tgId，value=member info，同时记录发言次数
            members_map = {}  # tgId -> {info + messageCount}

            # ── 模式1：getSupergroupMembers（获取管理员/机器人/最近活跃）──
            chat_result = self.tg.call_method('getChat', {'chat_id': chat_id})
            chat_result.wait(timeout=10)
            if chat_result.update:
                ...
                if chat_type.get('@type') == 'chatTypeSupergroup':
                    ...
                    if supergroup_id:
                        for sg_filter in [
                            ...
                            try:
                                sg_result = self.tg.call_method('getSupergroupMembers', {
                                    'supergroup_id': supergroup_id,
                                    'filter': sg_filter,
                                    'offset': 0,
                                    'limit': 200,
                                })
                                sg_result.wait(timeout=15)
                                if sg_result.update and sg_result.update.get('members'):
                                    for member in sg_result.update['members']:
                                        mid = member.get('member_id', {})
                                        if mid.get('@type') == 'messageSenderUser':
                                            uid = mid.get('user_id', 0)
                                            if uid and str(uid) not in members_map:
                                                info = self._get_sender_info(uid)
                                                display_name = info.get('first_name', '')
                                                if info.get('last_name'):
                                                    display_name += ' ' + info['last_name']
                                                members_map[str(uid)] = {
                                                    'tgId': str(uid),
            ...
            # ── 模式2：扫描消息历史提取发言用户（核心方案）──────────────
            from_message_id = 0
            scanned = 0
            batch_size = 100
            logger.info(f"[ACC{self.account_id}] 开始扫描消息历史提取用户, chat_id={chat_id}, 上限={scan_messages}条")

            while scanned < scan_messages:
                fetch_limit = min(batch_size, scan_messages - scanned)
                messages = []
                for _retry in range(4):
                    hist_result = self.tg.call_method('getChatHistory', {
                        'chat_id': chat_id,
                    ...
                    hist_result.wait(timeout=30)
                    if hist_result.error:
                        break
                    if hist_result.update:
                        messages = hist_result.update.get('messages', [])
                    if messages:
                        break
                ...
                for msg in messages:
                    ...
                    if sender.get('@type') == 'messageSenderUser':
                        ...
                        if uid and uid > 0:
                            uid_str = str(uid)
                            if uid_str in members_map:
                                members_map[uid_str]['messageCount'] = members_map[uid_str].get('messageCount', 0) + 1
                            else:
                                info = self._get_sender_info(uid)
                                display_name = info.get('first_name', '')
                                if info.get('last_name'):
                ...
                # 已达到用户数量上限则停止
                if len(members_map) >= limit:
                    break

                time.sleep(0.3)  # 限速，避免 flood

            logger.info(f"[ACC{self.account_id}] 消息扫描完成: 扫描{scanned}条消息, 发现{len(members_map)}个用户")

            # 按发言次数降序排序，取前 limit 个
            members_list = sorted(members_map.values(), key=lambda x: x['messageCount'], reverse=True)
            members_list = members_list[:limit]

            return {"success": True, "members": members_list, "total": len(members_list), "scanned": scanned}

        except Exception as e:
            logger.error(f"[ACC{self.account_id}] scrape_members 异常: {e}", exc_info=True)
            return {"success": False, "error": str(e)}

    def _handle_scrape_links(self, body: dict) -> dict:
        """采集群组消息中的链接和发送者 - 支持 chatId 或 group 参数"""
        chat_id = body.get("chatId")
        group = body.get("group")
        limit = body.get("limit", 100)
        # 如果传了 group（用户名），先解析为 chatId
        if not chat_id and group:
            try:
                username = group.lstrip("@").strip()
                if not username:
                    return {"success": False, "error": "group 参数为空"}
                # 方法1：searchPublicChat（最准确）
                try:
                    result = self.tg.call_method('searchPublicChat', {'username': username})
                    result.wait(timeout=15)
                    if result.update and result.update.get('id'):
                    ...
                    pass
                # 方法2：从 dialog_cache 中按 username 或 title 模糊匹配
                if not chat_id:
                    username_lower = username.lower()
                    with self._dialog_cache_lock:
                        for cid, info in self._dialog_cache.items():
                            ...
                            if cached_username == username_lower or username_lower in cached_title:
                                chat_id = cid
                                logger.info(f"[ACC{self.account_id}] 从 dialog_cache 匹配到群组: {info.get('title')} -> chatId={cid}")
                                break
                # 方法3：尝试 joinChat（有时能触发 TDLib 加载）
                if not chat_id:
                    try:
                        result = self.tg.call_method('searchChatsOnServer', {'query': username, 'limit': 5})
                        result.wait(timeout=10)
                        if result.update and result.update.get('chat_ids'):
                            chat_id = result.update['chat_ids'][0]
                    except Exception:
                        pass
                if not chat_id:
                    return {"success": False, "error": f"无法找到群组: {username}"}
        ...
        try:
            chat_id = int(chat_id)
            # 先 openChat 确保 TDLib 从服务器加载历史消息
            try:
                self.tg.call_method('openChat', {'chat_id': chat_id}).wait(timeout=5)
                time.sleep(1)  # 等待 TDLib 同步历史消息
            except:
                pass
            # 提取消息中的 t.me 链接
            tg_usernames = set()
            # 新增：提取消息发送者
            found_senders = {}  # {user_id: {info}}
            from_message_id = 0
            scanned = 0
            import re as _re
            while scanned < limit:
                ...
                for msg in result.update['messages']:
                    ...
                    if sender_type == 'messageSenderUser':
                        ...
                        if user_id and user_id not in found_senders:
                            # 记录发送者和消息时间
                            msg_date = msg.get('date', 0)
                            found_senders[user_id] = {'lastMsgDate': msg_date}
                        elif user_id and user_id in found_senders:
                            # 更新最后发言时间（取最新的）
                            msg_date = msg.get('date', 0)
                            if msg_date > found_senders[user_id].get('lastMsgDate', 0):
                                found_senders[user_id]['lastMsgDate'] = msg_date
                    elif sender_type == 'messageSenderChat':
                        # 频道/群组作为发送者（转发的频道消息）
                        fwd_chat_id = sender.get('chat_id', 0)
                        if fwd_chat_id and fwd_chat_id != chat_id:
                            # 尝试获取频道信息
                            try:
                                fwd_result = self.tg.call_method('getChat', {'chat_id': fwd_chat_id})
                                fwd_result.wait(timeout=3)
                                if fwd_result.update:
                                    fwd_username = self._extract_username_from_user(fwd_result.update) if hasattr(self, '_extract_username_from_user') else ''
                                    if fwd_username:
                                        tg_usernames.add(fwd_username.lower())
                            except:
                                pass
                    # === 新增：提取转发来源 ===
                    ...
                    if forward_info:
                        ...
                        if origin_type == 'messageOriginUser':
                            fwd_user_id = origin.get('sender_user_id', 0)
                            if fwd_user_id and fwd_user_id not in found_senders:
                                found_senders[fwd_user_id] = {'lastMsgDate': msg.get('date', 0)}
                        elif origin_type == 'messageOriginChannel':
                            # 转发自频道
                            fwd_chat_id2 = origin.get('chat_id', 0)
                            if fwd_chat_id2:
                                try:
                                    fwd_result2 = self.tg.call_method('getChat', {'chat_id': fwd_chat_id2})
                                    fwd_result2.wait(timeout=3)
                                    if fwd_result2.update:
                                        fwd_username2 = self._extract_username_from_user(fwd_result2.update) if hasattr(self, '_extract_username_from_user') else ''
                                        if fwd_username2:
                                            tg_usernames.add(fwd_username2.lower())
                                except:
                                    pass
                    # 从文本中提取 t.me 链接
                    text = self._extract_message_text(msg)
                    if text:
                        links = _re.findall(r'(?:https?://)?t\.me/([a-zA-Z][a-zA-Z0-9_]{3,31})', text)
                        for link in links:
                            if link.lower() not in ('joinchat', 'addstickers', 'setlanguage', 'proxy', 'socks', 'addtheme', 'iv'):
                                tg_usernames.add(link.lower())
                        # 也从 @mention 中提取用户名
                        mentions = _re.findall(r'@([a-zA-Z][a-zA-Z0-9_]{3,31})', text)
                        for mention in mentions:
                            if mention.lower() not in ('everyone', 'here', 'admin', 'admins'):
                                tg_usernames.add(mention.lower())
                    # 从 entities 中提取嵌入的 URL
                    msg_content = msg.get('content', {})
                    text_obj = msg_content.get('text', msg_content.get('caption', {}))
                    if isinstance(text_obj, dict):
                        entities = text_obj.get('entities', [])
                        for ent in entities:
                            ent_type = ent.get('type', {})
                            type_name = ent_type.get('@type', '')
                            url = ''
                            if type_name == 'textEntityTypeTextUrl':
                                url = ent_type.get('url', '')
                            elif type_name == 'textEntityTypeUrl':
                                # URL 在文本中，需要从 text 中截取
                                offset = ent.get('offset', 0)
                                length = ent.get('length', 0)
                                full_text = text_obj.get('text', '')
                                url = full_text[offset:offset+length]
                            if url:
                                m = _re.search(r'(?:https?://)?t\.me/([a-zA-Z][a-zA-Z0-9_]{3,31})', url)
                                if m:
                                    uname = m.group(1).lower()
                                    if uname not in ('joinchat', 'addstickers', 'setlanguage', 'proxy', 'socks', 'addtheme', 'iv'):
                                        tg_usernames.add(uname)
                            # 处理 @mention 类型的 entity
                            if type_name == 'textEntityTypeMention':
                                offset = ent.get('offset', 0)
                                length = ent.get('length', 0)
                                full_text = text_obj.get('text', '')
                                ...
                                if mention_text and len(mention_text) >= 4:
                                    tg_usernames.add(mention_text.lower())
            ...
            # === 批量获取发送者用户详情 ===
            sender_results = []
            sender_ids_list = list(found_senders.keys())
            for user_id in sender_ids_list:
                try:
                    user_result = self.tg.call_method('getUser', {'user_id': user_id})
                    user_result.wait(timeout=5)
                    if user_result.update:
                        user = user_result.update
                        uname = user.get('usernames', {}).get('editable_username', '') or ''
                        if not uname:
                            active_usernames = user.get('usernames', {}).get('active_usernames', [])
                            uname = active_usernames[0] if active_usernames else ''
                        if not uname:
                        ...
                        last_online = found_senders[user_id].get('lastMsgDate', 0)
                        if status_type == 'userStatusOnline':
                            last_online = int(time.time())
                        elif status_type == 'userStatusOffline':
                            was_online = status.get('was_online', 0)
                            if was_online > last_online:
                                last_online = was_online
                        elif status_type == 'userStatusRecently':
                            last_online = max(last_online, int(time.time()) - 86400)
                        # 如果用户有 username，从 tg_usernames 中移除（避免重复）
                        if uname:
                            tg_usernames.discard(uname.lower())
                        sender_results.append({
                            'type': 'user',
                            'tgId': str(user_id),
                            'username': uname,
                            'title': (fname + ' ' + lname).strip(),
                            'memberCount': 0,
                            'description': '',
                            'isPremium': is_premium,
                            'isBot': is_bot,
                            'lastOnline': last_online,
                            'lastMsgDate': found_senders[user_id].get('lastMsgDate', 0),
                        })
                except Exception:
                    continue
                time.sleep(0.05)  # 轻量延迟避免 flood
            # 解析 t.me/ 链接的详细信息（限制50个，避免超时）
            link_results = []
            for uname in list(tg_usernames)[:50]:
                try:
                    chat_result = self.tg.call_method('searchPublicChat', {'username': uname})
                    chat_result.wait(timeout=10)
                    if not chat_result.update:
                        continue
                    chat_info = chat_result.update
                    chat_type_obj = chat_info.get('type', {})
                    chat_type_name = chat_type_obj.get('@type', '')
                    entry = {
                        'tgId': str(chat_info.get('id', '')),
                        'username': uname,
                    ...
                    if chat_type_name == 'chatTypeSupergroup':
                        sg_id = chat_type_obj.get('supergroup_id', 0)
                        is_channel = chat_type_obj.get('is_channel', False)
                        entry['type'] = 'channel' if is_channel else 'group'
                        try:
                            sg_result = self.tg.call_method('getSupergroup', {'supergroup_id': sg_id})
                            sg_result.wait(timeout=5)
                            if sg_result.update:
                                entry['memberCount'] = sg_result.update.get('member_count', 0)
                                entry['description'] = sg_result.update.get('description', '') or ''
                        except:
                            pass
                    elif chat_type_name == 'chatTypePrivate':
                        entry['type'] = 'user'
                        user_id = chat_type_obj.get('user_id', 0)
                        try:
                            user_result = self.tg.call_method('getUser', {'user_id': user_id})
                            user_result.wait(timeout=5)
                            if user_result.update:
                                entry['isPremium'] = user_result.update.get('is_premium', False)
                                entry['isBot'] = user_result.update.get('type', {}).get('@type') == 'userTypeBot'
                                fname = user_result.update.get('first_name', '')
                                lname = user_result.update.get('last_name', '')
                                entry['title'] = (fname + ' ' + lname).strip()
                                status = user_result.update.get('status', {})
                                status_type = status.get('@type', '')
                                if status_type == 'userStatusOnline':
                                    entry['lastOnline'] = int(time.time())
                                elif status_type == 'userStatusOffline':
                                    entry['lastOnline'] = status.get('was_online', 0)
                                elif status_type == 'userStatusRecently':
                                    entry['lastOnline'] = int(time.time()) - 86400
                                elif status_type == 'userStatusLastWeek':
                                    entry['lastOnline'] = int(time.time()) - 604800
                                elif status_type == 'userStatusLastMonth':
                                    entry['lastOnline'] = int(time.time()) - 2592000
                        except:
                            pass
                    elif chat_type_name == 'chatTypeBasicGroup':
                        entry['type'] = 'group'
                        bg_id = chat_type_obj.get('basic_group_id', 0)
                        try:
                            bg_result = self.tg.call_method('getBasicGroup', {'basic_group_id': bg_id})
                            bg_result.wait(timeout=5)
                            if bg_result.update:
                                entry['memberCount'] = bg_result.update.get('member_count', 0)
                        except:
                            pass
                    ...
                    continue
            # 合并结果：发送者 + 链接解析结果
            results = sender_results + link_results
            return {"success": True, "results": results, "total": len(results), "scanned": scanned, "senderCount": len(sender_results), "linkCount": len(link_results)}
        except Exception as e:
            return {"success": False, "error": str(e)}
